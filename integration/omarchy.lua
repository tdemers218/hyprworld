...
local function customizer_shortcut(name, fallback)
    local file = io.open((os.getenv("HOME") or "") .. "/.config/omarchy/hyprworld.json", "r")
        or io.open((os.getenv("HOME") or "") .. "/.config/omarchy/plugins/io.github.tdemers218.hyprworld/customizer.json", "r")
...
local function launcher_handled(delta)
    ...
    local command = "omarchy-shell shell call " .. target .. " moveSelected " .. delta
...
o.bind(overview_key,"Hyprworld: Toggle overview",function()
